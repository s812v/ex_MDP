...
class MainTask2:
    ...
    def __init__(self, image_processing_server_url: str=None):
        ...
        self.commands_queue = self.manager.DequeProxy(["P", "S", "P"]) # * queue for commands within robot, firing commands to arduino
        ...
        if source_header == IMAGESERVER_HEADER:
            if target_header == RASPBERRY_HEADER:
                if data_type == "Detections":
                    ...
                    if image_character == "0":
                        command = "R"
                    elif image_character == "1":
                        command = "L"
                    if command != None:
                        self.to_arduino_message_deque.append(command)
                        # self.status["state"] = 'Idle'
                    else:
                        print("invalid image recognition left or right only ", image_character)
                    routed = True
                ...
                if self.status["state"] == 'Idle':
                    if len(self.commands_queue) > 0:
                        ...
                        if command == 'P':
                            ...
                            if self.status["dummy_image"]:
                                self.status["dummy_image"] = False
                                pass # just ignore, and pass S 
                            else:  
