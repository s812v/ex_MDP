...
class Algorithm_communicator:
    # Passively listening to (waiting or already) not yet run algorithm client, 
    # run main.py first, then start all the clients
    def __init__(self, host=WIFI_IP, port=WIFI_PORT):
        ...
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # local socket

        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # local socket
...
class Algorithm_communicator_client:
    # Active connecting to (waiting or already) running algorithm, run main.py first, then start all the clients
    def __init__(self, host=WIFI_IP, port=WIFI_PORT):
