...
class Status(object):
    ...
    def set_state(self, state):
        self.status.set_last_state(self.state)
        self._state = state
...
class NavigateSearching:
    ...
    def _image_process(self):
        ...
        while True:
            try:
                image =  self.image_deque.popleft()
                message = self._routing(IncomingMessage(
                    source_header=RASPBERRY_HEADER,
                    target_header=ALGORITHM_HEADER,
                    data_type="Image",data=image
                ))
            except Exception as error:
    ...
    def _status_process(self):
        last_state = self.status.last_state
        while True:
